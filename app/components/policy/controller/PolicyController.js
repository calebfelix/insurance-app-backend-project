...
class PolicyController {
  ...
  async createPolicy(settingsConfig, req, res, next) {
    try {
      ...
      const { planId,insuranceTypeId } = req.params;
      const policyIds = v4();
      ...
      let insuranceData = await this.insuranceTypeService.getAllInsuranceTypeById(
        settingsConfig,
        insuranceTypeId,
      );
      console.log(insuranceData.rows[0].dataValues.insuranceName)
      if (insuranceData.rows[0].length == 0) {
        throw new Error("Insurance Type Not Found");
      }

      validateUuid(planId);
      let { rows, count } = await this.planService.getAllPlansById(
        settingsConfig,
        insuranceTypeId,
        planId,
        req.query
      ...
      req.body.insuranceType = insuranceData.rows[0].dataValues.insuranceName;
      req.body.planName = insuranceData.rows[0].dataValues.insuranceName;
      ...
      req.body.agentId = customer[0].agentId;
      ...
      console.log(years, rows[0].policyTermMin, rows[0].policyTermMax)
      ...
      paymentBody.agentId = customer[0].agentId;
