...
class EmployeeController {
    ...
    async createEmployee(settingsConfig, req, res, next) {
        try {
            ...
            const { employeeName, username, password, email,employeeImgUrl } = newBody
    ...
    async SendEmail(settingsConfig, req, res, next) {
        console.log("hi");
        try {
            const response = await axios.post(
                `http://localhost:3001/api/v1/cronjob`, { headers: { auth: "auth" } }
    ...
    async getAllEmpoyeeEmail(settingsConfig, req, res, next) {
        try {
            const logger = settingsConfig.logger;
            const queryParams = req.query
            logger.info(`[EMPLOYEE_CONTROLLER] : Inside getAllEmpoyeeEmail`);
